...
1. 使用 `E:\AICode\DD_Rag_clean` 作为发布仓库。
2. 不要推送 `E:\AICode\DD_Rag` 的原始 Git 历史。
3. 提交前扫描敏感信息。
4. 确认 `.env`、密钥、token、真实密码没有进入 Git。
5. 文档和 docker 部署文件应保留，因为它们是当前可复现部署的关键材料。

当前最重要的安全提醒：

- 原始历史中出现过 DashScope API Key，应该在控制台轮换。
- clean 仓库不携带旧历史，但仍要在每次提交前重新扫描。
