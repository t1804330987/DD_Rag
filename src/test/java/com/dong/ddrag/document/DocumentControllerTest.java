...
class DocumentControllerTest {
    ...
    static void registerDatasourceProperties(DynamicPropertyRegistry registry) {
        ...
        registry.add("ddrag.dev-admin.enabled", () -> "false");
    }
    ...
    @Test
    void shouldRetryFailedDocumentIngestionForGroupOwner() throws Exception {
        long documentId = uploadDocument("u1001", 2001L, "重试解析文档.txt");
        updateDocumentStatusAndFailure(documentId, "FAILED", "向量写入失败");

        mockMvc.perform(org.springframework.test.web.servlet.request.MockMvcRequestBuilders.post(
                        "/api/documents/{documentId}/retry-ingestion", documentId)
                        .param("groupId", "2001")
                        .header(HttpHeaders.AUTHORIZATION, bearerToken(1001L, "u1001")))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.success").value(true));
    ...
    void shouldFilterReadableDocumentsAcrossOwnedAndJoinedGroups() throws Exception {
        ...
        updateDocumentStatus(joinedDocumentId, "READY");
        updateDocumentStatus(ownedReadyDocumentId, "READY");
    ...
    private void updateDocumentStatusAndFailure(long documentId, String status, String failureReason) {
        jdbcTemplate.update(
                "update documents set status = ?, failure_reason = ?, updated_at = now() where id = ?",
                status,
                failureReason,
                documentId
        );
