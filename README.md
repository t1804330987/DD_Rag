...
- [src/main/resources/application-dev.yml](E:\AICode\DD_Rag_clean\src\main\resources\application-dev.yml:19)
  将 `spring.ai.dashscope.api-key` 改成你自己的 DashScope API Key。
- [docker-compose.yml](E:\AICode\DD_Rag_clean\docker-compose.yml:97)
  将 `backend.environment.DASHSCOPE_API_KEY` 改成同一个 DashScope API Key。

如果你还需要调整本机端口或容器访问地址，也需要同步检查：

- [src/main/resources/application-dev.yml](E:\AICode\DD_Rag_clean\src\main\resources\application-dev.yml:9)
  本地开发直连数据库、Ollama、Elasticsearch、MinIO 的地址。
- [docker-compose.yml](E:\AICode\DD_Rag_clean\docker-compose.yml:98)
  容器内 backend 连接 PostgreSQL、MinIO、Ollama、Elasticsearch 的地址。

当前仓库默认把 `DD_RAG_JWT_SECRET` 也写在 `docker-compose.yml` 里，单机启动时无需额外准备环境变量。
